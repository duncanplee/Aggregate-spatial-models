...
// [[Rcpp::export]]
NumericVector vcmcompute(List weights_list, NumericVector vcm, NumericVector x_grid, const int K)
{
    // Create a vector to save the results in
    NumericVector vcmcomp(K);
    
    // Compute the VCM part
    for(int i = 0; i < K; i++)
    {
        NumericMatrix temp = weights_list[i];
        NumericVector temp2 = temp(_,0) - 1;
        NumericVector temp3 = x_grid[temp2];
        vcmcomp[i] = sum(temp(_, 1) * exp(vcm[i] * temp3));
    }
