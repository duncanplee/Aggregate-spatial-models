poisson.lerouxCARaggregatevcm <- function(formula, data=NULL,  x.grid, EM, weights, W, burnin, n.sample, thin=1, prior.mean.beta=NULL, prior.var.beta=NULL, prior.mean.alpha=NULL, prior.var.alpha=NULL, prior.tau2=NULL, rho=NULL, MALA=TRUE, verbose=TRUE)
{
##############################################
...
#### Compute a list object for the weights
M <- ncol(weights)
weights.list <- as.list(rep(NA, K))
    for(i in 1:K)
    {
    non.zero <- which(weights[i, ]!=0)
    weights.list[[i]] <- cbind(non.zero, weights[i, non.zero])
    }
...
#### Priors
  ...
  if(is.null(prior.mean.alpha)) prior.mean.alpha <- c(0, 0)
  if(is.null(prior.var.alpha)) prior.var.alpha <- c(100000, 100000)  
  if(is.null(prior.tau2)) prior.tau2 <- c(1, 0.01)
common.prior.beta.check(prior.mean.beta, prior.var.beta, p)
common.prior.beta.check(prior.mean.alpha, prior.var.alpha, 2)
...
#### Compute the average value for x.grid
weights.spam <- as.spam(weights)
x.grid.mean <- mean(x.grid)
x.grid.standardised <- x.grid - x.grid.mean
x.grid.av <- weights.spam %*% x.grid.standardised
...
mod.glm <- glm(Y~X.standardised-1 + x.grid.av, offset=offset, family="quasipoisson")
...
#### Compute the VCM part
alpha.vcm <- alpha[1] + alpha[2] * EM
grid.current <- vcmcompute(weights_list=weights.list, vcm=alpha.vcm, x_grid=x.grid.standardised, K=K)


#### Random effects 
log.Y <- log(Y)
log.Y[Y==0] <- -0.1  
res.temp <- log.Y - X.standardised %*% beta.mean[1:p] - offset - log(grid.current)
res.sd <- sd(res.temp, na.rm=TRUE)/5
phi <- rnorm(n=K, mean=rep(0,K), sd=res.sd)
tau2 <- var(phi) / 10
fitted <- exp(as.numeric(X.standardised %*% beta) + offset + phi) * grid.current
...
samples.alpha <- array(NA, c(n.keep, 2))
...
#### Create the MCMC samples    
  for(j in 1:n.sample)
  {
    ...
    ## Sample from beta
    ####################
    offset.temp <- phi + offset + log(grid.current)
    if(p>2)
    {
    ...
    ####################
    #### Propose a new value
    alpha.proposal <- rnorm(n=2, mean=alpha, sd=proposal.sd.alpha)
    
    #### Compute the fitted values
    offset.alpha <- as.numeric(X.standardised %*% beta) + offset + phi
    alpha.proposal.vcm <- alpha.proposal[1] + alpha.proposal[2] * EM
    grid.proposal <- vcmcompute(weights_list=weights.list, vcm=alpha.proposal.vcm, x_grid=x.grid.standardised, K=K)
    fitted.current <- exp(offset.alpha) * grid.current
    fitted.proposal <- exp(offset.alpha) * grid.proposal
    
    #### Compute the likelihood ratio
    fc.current <- sum(Y.DA * log(fitted.current) - fitted.current) - sum((alpha - prior.mean.alpha)^2 / (2 * prior.var.alpha))
    fc.proposal <- sum(Y.DA * log(fitted.proposal) - fitted.proposal) - sum((alpha.proposal - prior.mean.alpha)^2 / (2 * prior.var.alpha)) 
    prob.alpha <- exp(fc.proposal - fc.current)
    
    #### Accept or reject the proposal
    if(prob.alpha > runif(1))
    {
        alpha <- alpha.proposal
        grid.current <- grid.proposal
        accept[7] <- accept[7] + 1           
    }else
    {}              
    accept[8] <- accept[8] + 1           
    ...
    ## Sample from phi
    ####################
    beta.offset <- regression + offset + log(grid.current)
    if(MALA)
    {
    ...
    #########################
    lp <- as.numeric(regression) + phi + offset + log(grid.current)
    fitted <- exp(lp)
    ...
    if(ceiling(k)==floor(k))
    {
      ...
      proposal.sd.alpha <- common.accceptrates1(accept[7:8], proposal.sd.alpha, 40, 50)
  ...
  accept.tau2 <- 100
  accept.final <- c(accept.beta, accept.alpha, accept.phi, accept.rho, accept.tau2)
  names(accept.final) <- c("beta", "alpha", "phi", "rho", "tau2")
  
  
  #### Compute the fitted deviance
  mean.beta <- apply(samples.beta, 2, mean)
  mean.alpha <- apply(samples.alpha, 2, mean)
  mean.phi <- apply(samples.phi, 2, mean)
  alpha.mean.vcm <- mean.alpha[1] + mean.alpha[2] * EM
  grid.mean <- vcmcompute(weights_list=weights.list, vcm=alpha.mean.vcm, x_grid=x.grid.standardised, K=K)
  fitted.mean <- exp(X.standardised %*% mean.beta + mean.phi + offset) * grid.mean
  ...
  summary.hyper[1:2, 1:3] <- t(apply(samples.alpha, 2, quantile, c(0.5, 0.025, 0.975))) 
  summary.hyper[1:2, 4:7] <- cbind(rep(n.keep,2), rep(accept.alpha,2), effectiveSize(samples.alpha), geweke.diag(samples.alpha)$z)
  summary.hyper[3, 1:3] <- quantile(samples.tau2, c(0.5, 0.025, 0.975))
  summary.hyper[3, 4:7] <- c(n.keep, accept.tau2, effectiveSize(samples.tau2), geweke.diag(samples.tau2)$z)
  if(!fix.rho)
  {
  ...
  rownames(summary.results)[(nrow(summary.results)-3):nrow(summary.results)] <- c("alpha intercept", "alpha slope", "tau2", "rho")
  ...
  model.string <- c("Likelihood model - Poisson (log link function) with grid level aggregation and linear VCM", "\nRandom effects model - Leroux CAR\n")
  if(fix.rho) samples.rho=NA
  if(n.miss==0) samples.Y = NA
  samples <- list(beta=samples.beta.orig, alpha=samples.alpha, phi=mcmc(samples.phi), tau2=mcmc(samples.tau2), rho=mcmc(samples.rho), fitted=mcmc(samples.fitted), Y=mcmc(samples.Y))
