poisson.lerouxCARaggregate <- function(formula, data=NULL,  x.grid, weights, W, burnin, n.sample, thin=1, prior.mean.beta=NULL, prior.var.beta=NULL, prior.mean.alpha=NULL, prior.var.alpha=NULL, prior.tau2=NULL, rho=NULL, MALA=TRUE, verbose=TRUE)
{
##############################################
...
#### Priors
  ...
  if(is.null(prior.var.alpha)) prior.var.alpha <-100000  
  if(is.null(prior.tau2)) prior.tau2 <- c(1, 0.01)
common.prior.beta.check(prior.mean.beta, prior.var.beta, p)
common.prior.beta.check(prior.mean.alpha, prior.var.alpha, 1)
...
x.grid.sd <- sd(x.grid)
x.grid.standardised <- (x.grid - x.grid.mean) / x.grid.sd
...
log.Y <- log(Y)
log.Y[Y==0] <- -0.1  
res.temp <- log.Y - X.standardised %*% beta.mean[1:p] - offset - alpha * x.grid.av
res.sd <- sd(res.temp, na.rm=TRUE)/5
phi <- rnorm(n=K, mean=rep(0,K), sd=res.sd)
tau2 <- var(phi) / 10
fitted <- exp(as.numeric(X.standardised %*% beta) + offset + phi) * weights.spam %*%exp(x.grid.standardised * alpha)
grid.current <- weights.spam %*%exp(x.grid.standardised * alpha) 
...
#### Create the MCMC samples    
  for(j in 1:n.sample)
  {
    ...
    grid.proposal <- weights.spam %*% exp(x.grid.standardised * alpha.proposal) 
    fitted.current <- exp(offset.alpha) * grid.current
    fitted.proposal <- exp(offset.alpha) * grid.proposal
    
    #### Compute the likelihood ratio
    fc.current <- sum(Y.DA * log(fitted.current) - fitted.current) - (alpha - prior.mean.alpha)^2 / (2 * prior.var.alpha) 
    fc.proposal <- sum(Y.DA * log(fitted.proposal) - fitted.proposal) - (alpha.proposal - prior.mean.alpha)^2 / (2 * prior.var.alpha) 
  ...
  fitted.mean <- exp(X.standardised %*% mean.beta + mean.phi + offset) * weights.spam %*% exp(x.grid.standardised * mean.alpha) 
  ...
  samples.beta.orig[ ,which(X.indicator==2)] <-  samples.beta.orig[ ,which(X.indicator==2)] - x.grid.mean / x.grid.sd * samples.alpha
  samples.alpha.orig <- samples.alpha / x.grid.sd
  
  
  #### Create a summary object
  samples.beta.orig <- mcmc(samples.beta.orig)
  samples.alpha.orig <- mcmc(samples.alpha.orig)
  ...
  summary.hyper[1, 1:3] <- quantile(samples.alpha.orig, c(0.5, 0.025, 0.975))
  summary.hyper[1, 4:7] <- c(n.keep, accept.alpha, effectiveSize(samples.alpha.orig), geweke.diag(samples.alpha.orig)$z)
  ...
  rownames(summary.results)[(nrow(summary.results)-2):nrow(summary.results)] <- c("Grid covariate", "tau2", "rho")
  ...
  model.string <- c("Likelihood model - Poisson (log link function) with grid level aggregation", "\nRandom effects model - Leroux CAR\n")
  if(fix.rho) samples.rho=NA
  if(n.miss==0) samples.Y = NA
  samples <- list(beta=samples.beta.orig, alpha=samples.alpha.orig, phi=mcmc(samples.phi), tau2=mcmc(samples.tau2), rho=mcmc(samples.rho), fitted=mcmc(samples.fitted), Y=mcmc(samples.Y))
